...
class LandmarkDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Generate ground truth one-hot maps, classification maps, and heatmap
        channels = np.zeros([self.cfg_dataset.KEY_POINTS, image.shape[0], image.shape[1]])
        # heatmap_channels = np.zeros([self.cfg_dataset.KEY_POINTS, image.shape[0], image.shape[1]])
        texture_channels = np.zeros([self.cfg_dataset.KEY_POINTS, image.shape[0], image.shape[1]])
        strcture_channels = np.zeros([1, image.shape[0], image.shape[1]])
        for i in range(self.cfg_dataset.KEY_POINTS):
            x, y = np.mean(landmarks_per_annotator[:, i], axis=0).astype(int)
            channels[i, y, x] = 1.0
            texture_channels[i] = makeGaussian(image.shape, 20, (x,y))
        # Add a channel to the images
        image = np.expand_dims(image, axis=0)
        strcture_channels[0] = np.max(texture_channels, axis=0)
        ...
        # addition info
        meta["texture_channels"] = texture_channels
        meta["strcture_channels"] = strcture_channels
